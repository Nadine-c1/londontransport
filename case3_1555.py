...
def load_fiets_data():
    ...
    fiets_df = pd.concat([F4_dec, F6_mrt, F30_okt, F2_jan, F26_jun, F21_aug])
    ...
    fiets_start = fiets_df.groupby(['Start', 'StartStation Name', 'StartStation Id','week_soort']).size().reset_index(name="Vertrek_Count")
    fiets_eind = fiets_df.groupby(['End', 'EndStation Name', 'EndStation Id','week_soort']).size().reset_index(name="Aankomst_Count")
    fiets_count = fiets_start.merge(fiets_eind, left_on=['Start', 'StartStation Name'], right_on=['End', 'EndStation Name'], how='outer').fillna(0)
    fiets_count.rename(columns={'Start': 'datum', 'StartStation Name': 'Station', 'StartStation Id': 'Id','week_soort_x':'Week_soort'}, inplace=True)
...
# barplot
    # Zet de juiste kolommen om en groeperen
